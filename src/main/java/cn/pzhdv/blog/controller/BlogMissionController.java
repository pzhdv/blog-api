...
public class BlogMissionController {
    ...
    private final BlogMissionService service;
    ...
    public Result<BlogMission> list() {
        // 先从 Redis 中获取缓存数据
        BlogMission blogMission = redisUtils.get(RedisKey.BLOG_MISSION_CACHE_KEY, BlogMission.class);
        if (blogMission == null) {
            blogMission = service.queryBlogMission();
            redisUtils.set(RedisKey.BLOG_MISSION_CACHE_KEY, blogMission);
        }
